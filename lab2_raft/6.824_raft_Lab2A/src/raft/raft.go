...
type Raft struct {
	...
	commitIndex int
	nextIndex []int
	applyCh chan ApplyMsg   	// 提交成功就向这个管道传输信息
...
func (rf *Raft) election() {
	...
	for peer := range rf.peers {
		...
		DPrintf("[Node %d] 选举开始。。。", rf.me)
		go func(args RequestVoteArgs, reply RequestVoteReply, peer int) {
			DPrintf("【Node %d】开始向 %d发送vote请求", rf.me, peer)
			if rf.sendRequestVote(peer, &args, &reply) {
				rf.mu.Lock()
				defer rf.mu.Unlock()
				if reply.VoteGranted {
					DPrintf("[Node %d] 投票给 [Node %d]", peer, rf.me)
					voteCount++
				} else {
					if reply.Term > rf.currentTerm {
						rf.currentTerm = reply.Term
						rf.voteFor = -1
						rf.state = Follower // 遇到任期号大的转变为follwer
					}
					return
				}
				if voteCount > len(rf.peers)/2 && flag && rf.state == Candidate {
					DPrintf("[Node %d]成为leader", rf.me)
					flag = false
					rf.state = Leader
					rf.beatHeart()
				}
			} else {
				DPrintf("[Node %d] to [Node %d] 网络不通，投票失败", args.CandidateId, peer)
			}
		}(args, reply, peer)
	}
}
...
func (rf *Raft) RequestBeatHeart(args *BeatHeartArgs, reply *BeatHeartReply) {
	...
	if args.IsAppend {
		if rf.match(args.PrevLogIndex, args.PrevLogTerm) {
			rf.logs = append(rf.logs, args.AppendLog) // 追加新日志
			reply.Success = true
		} else {
			reply.Success = false
		}
	...
	rf.electionTime = time.Now() // 防止其选举
}
...
func (rf *Raft) beatHeart() {
	...
	for peer := range rf.peers {
		...
		if rf.nextIndex[peer] < len(rf.logs) - 1 {
			...
			args.IsAppend = true // 需要日志复制的心跳
		} else {
			args.IsAppend = false // 普通心跳
		}
		...
		go func(args BeatHeartArgs, reply BeatHeartReply, peer int) {
			if rf.sendBeatHeart(peer, &args, &reply) {
				...
				defer rf.mu.Unlock()
				if reply.Term > rf.currentTerm {
					rf.state = Follower // 不能再发心跳了
					return
				}
				if !reply.Success {
					rf.nextIndex[peer]-- // 没发成功只能发前一个
				} else {
					rf.nextIndex[peer]++ // 发成功了，接下来需要发下一个
				}
			} else {
				DPrintf("[Node %d] to [Node %d] 网络不通，心跳失败", args.LeaderId, peer)
			}
		}(args, reply, peer)
	}
